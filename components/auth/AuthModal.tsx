...
export default function AuthModal() {
  ...
  async function submitAuth(e: FormEvent<HTMLFormElement>) {
    ...
    if (activeTab === "guest") {
      const guestName = target.elements.namedItem(
        "guestName"
      ) as HTMLInputElement;
      ...
      const loginEmail = target.elements.namedItem(
        "loginEmail"
      ) as HTMLInputElement;
      const loginPassword = target.elements.namedItem(
        "loginPassword"
      ) as HTMLInputElement;
      if (
        !loginEmail ||
      ...
      const registerName = target.elements.namedItem(
        "registerName"
      ) as HTMLInputElement;
      const registerEmail = target.elements.namedItem(
        "registerEmail"
      ) as HTMLInputElement;
      const registerPassword = target.elements.namedItem(
        "registerPassword"
      ) as HTMLInputElement;
      if (
        !registerName ||
      ...
      >
        <label className="modal-box flex max-w-sm flex-col gap-4 pt-2">
          {session?.user?.id && typeof session.user.id === "number" ? (
            ...
            <>
              <div className="flex-nowap tabs w-full">
                <span
                  onClick={() => {
                    setActiveTab("guest");
                  }}
                  className={
                    "tab-bordered tab tab-border-2 flex-grow rounded-tl-lg" +
                    (activeTab === "guest"
                      ? " tab-active text-base-content"
                      : " border-opacity-10 text-base-content hover:border-opacity-30")
                  }
                >
                  Guest
                </span>
              ...
              <form className="flex flex-col px-2" onSubmit={submitAuth}>
                ...
                {serverMessage && (
                  <div className="mt-2 text-error">{serverMessage}</div>
                )}
                <div className="modal-action items-center">
                  ...
                  <button
                    className={"btn" + (buttonLoading ? " loading" : "")}
                    type="submit"
                  >
